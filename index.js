...
const main = async () => {
  ...
  app.get('/getTokens', cookieParser(), async (req, res) => {
    try {
      ...
      const userInlBalance = await xrpl.send({ command: 'gateway_balances', account: account })
      if (userInlBalance.assets[config.issuer]) {
        let { value } = userInlBalance.assets[config.issuer][0]
        assert(Number(value) > 1, `This account must hold atleast 1 ${config.token} to claim more INL tokens. Contact admin for more info.`)
        assert(Number(value) <= 200, `You are the strong ${config.token} holder, thank you for your support. You need to know that ${config.token} faucet was created for small holders.`)
      }
